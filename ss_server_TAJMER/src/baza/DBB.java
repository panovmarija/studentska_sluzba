...
//    METODE KOJE RADE NA OSVEZAVENJU TABELE
//    NEOPHODNO JE DA METODE UCITAVAJU PODATKE NOVOM KONEKCIJOM SA AUTOCOMMIT(TRUE)
    public List<Wrapper> vratiBroj() {
        
            List<Wrapper> l=new LinkedList<>();
        try {
              String url = "jdbc:mysql://localhost:3316/knjiga";
              Connection novaKonekcija = DriverManager.getConnection(url, "root", "root");
              novaKonekcija.setAutoCommit(true);  // obavezno da vidiš ažurne podatke
              String u = "select zvanje,count(*) broj from prof group by zvanje ";
              Statement s = novaKonekcija.createStatement();
    ...
    public List<Wrapprof> vratiProf_i_brAng() {
        ...
        try {
            Connection novaConn=DriverManager.getConnection("jdbc:mysql://localhost:3316/knjiga", "root", "root");
            novaConn.setAutoCommit(true);
            String u = "SELECT  p.id,p.ime, p.prez, p.zvanje ,COUNT(*) broj FROM prof p LEFT JOIN ang a ON(a.prof=p.id) GROUP BY p.id,p.ime, p.prez, p.zvanje";
             Statement s=novaConn.createStatement();
            ResultSet rs=s.executeQuery(u);
            while(rs.next())
            {
